"""
Extract pulp and paper calibration data and save to CIMS-formatted CSV files.

Sources
-------
Emissions  (calibration_emissions_total from crosswalk; calibration_emissions_by_type from nir_to_cims)
    nir_crosswalk_tables_cims.py  → total tCO2e per pulp and paper CIMS branch
                                    5-year intervals (2000–2020);
                                    abbreviation regions (AB, BC, …)
    nir_to_cims.py                → per-gas kt per pulp and paper CIMS branch,
...
# ── configuration ─────────────────────────────────────────────────────────────
OUTPUT_DIR = BASE_PATH / 'calibration/pulp_and_paper'
...
def _build_cer_energy(cer_df: pd.DataFrame) -> pl.DataFrame:
    """Filter cer_resd_demand output to Pulp and Paper CIMS nodes."""
    pp = cer_df[cer_df['Node'].str.startswith('.Pulp and Paper')].copy()
    if pp.empty:
        return _empty_df()
...
def _build_crosswalk_emissions(crosswalk_df: pl.DataFrame) -> pl.DataFrame:
    """Filter nir_crosswalk_tables_cims output to Pulp and Paper CIMS branches."""
    pp = crosswalk_df.filter(pl.col('CIMS_Branch').str.contains(r'\.Pulp and Paper'))
    if pp.is_empty():
        return _empty_df()

    rows = []
    for row in pp.to_dicts():
...
def _build_nir_emissions(nir_df: pl.DataFrame) -> pl.DataFrame:
    """Extract per-gas NIR emissions for Pulp and Paper branches."""
    known_regions = set(_REGION_MAP.keys())
    pp = nir_df.filter(
        pl.col('CIMS Branch').str.contains(r'\.Pulp and Paper')
...
def main() -> pl.DataFrame:
    """Assemble pulp and paper calibration data and write one CSV per region."""
    print('=' * 60)
    print('PULP AND PAPER CALIBRATION')
    print('=' * 60)
    ...
    print('Filtering to regions with Pulp and Paper (see sector_region_map.csv)...')
    ...
    if allowed_regions:
        ...
        if dropped_count:
            print(f'  Dropped {dropped_count:,} rows for regions without Pulp and Paper')
    ...
    print(f'\n✅ Pulp and paper calibration complete')
