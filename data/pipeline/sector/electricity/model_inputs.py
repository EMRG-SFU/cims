...
Fixed structural parameters
    ...
    Encodes the full Electricity hierarchy:
      Electricity (Sector)
        └── Utility Generation (Service, Tech Compete)
        │     ├── Base Load  (Service, Tech Compete)
        │     ├── Shoulder Load (Service, Tech Compete)
        │     └── Peak Load (Service, Tech Compete)
        ├── Transmission (Service, Tech Compete)
        ├── Storage (Service, Tech Compete)
        │     ├── Battery (Service, Tech Compete)
        │     └── Seasonal (Service, Tech Compete)
        └── CCS (Service, Tech Compete)
              ├── CCS_Biomass (Service, Tech Compete)
              ├── CCS_Coal (Service, Tech Compete)
              └── CCS_Natural Gas (Service, Tech Compete)
...
Energy price multipliers  (multiplier_price rows)
    ...
    Electricity is classified as Industrial in sector_map.csv.
...
def _read_flattened_fixed() -> pl.DataFrame:
    """Flatten all Electricity fixed CSVs and return combined DataFrame."""
...
def _build_price_rows(multipliers: pl.DataFrame) -> pl.DataFrame:
    ...
    return (
        ...
        .select([
            (pl.lit('CIMS.CAN.') + pl.col('Region') + pl.lit('.Electricity')).alias('Branch'),
...
def main() -> pl.DataFrame:
    ...
    print(f'\n✅ Electricity model inputs complete')
