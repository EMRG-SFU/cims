"""
Extract electricity calibration data and save to CIMS-formatted CSV files.

Sources
-------
Emissions  (calibration_emissions_total from crosswalk; calibration_emissions_by_type from nir_to_cims)
    nir_crosswalk_tables_cims.py  → total tCO2e per electricity CIMS branch
                                    5-year intervals (2000–2020);
                                    abbreviation regions (AB, BC, …)
    nir_to_cims.py                → per-gas kt per electricity CIMS branch,
...
# ── configuration ─────────────────────────────────────────────────────────────
OUTPUT_DIR = BASE_PATH / 'calibration/electricity'
...
def _build_cer_energy(cer_df: pd.DataFrame) -> pl.DataFrame:
    """Filter cer_resd_demand output to Electricity CIMS nodes."""
    elec = cer_df[cer_df['Node'].str.startswith('.Electricity')].copy()
    if elec.empty:
        return _empty_df()
...
def _build_crosswalk_emissions(crosswalk_df: pl.DataFrame) -> pl.DataFrame:
    """Filter nir_crosswalk_tables_cims output to Electricity CIMS branches."""
    elec = crosswalk_df.filter(pl.col('CIMS_Branch').str.contains(r'\.Electricity'))
    if elec.is_empty():
        return _empty_df()

    rows = []
    for row in elec.to_dicts():
...
def _build_nir_emissions(nir_df: pl.DataFrame) -> pl.DataFrame:
    """Extract per-gas NIR emissions for Electricity branches."""
    known_regions = set(_REGION_MAP.keys())
    elec = nir_df.filter(
        pl.col('CIMS Branch').str.contains(r'\.Electricity')
...
def main() -> pl.DataFrame:
    """Assemble electricity calibration data and write one CSV per region."""
    print('=' * 60)
    print('ELECTRICITY CALIBRATION')
    print('=' * 60)
    ...
    print('Filtering to regions with Electricity (see sector_region_map.csv)...')
    ...
    if allowed_regions:
        ...
        if dropped_count:
            print(f'  Dropped {dropped_count:,} rows for regions without Electricity')
    ...
    print(f'\n✅ Electricity calibration complete')
