"""
Extract petroleum refining calibration data and save to CIMS-formatted CSV files.

Sources
-------
Emissions  (calibration_emissions_total from crosswalk; calibration_emissions_by_type from nir_to_cims)
    nir_crosswalk_tables_cims.py  → total tCO2e per petroleum refining CIMS branch
                                    5-year intervals (2000–2020);
                                    abbreviation regions (AB, BC, …)
    nir_to_cims.py                → per-gas kt per petroleum refining CIMS branch,
...
# ── configuration ─────────────────────────────────────────────────────────────
OUTPUT_DIR = BASE_PATH / 'calibration/petroleum_refining'
...
SECTOR_NAME = 'Petroleum Refining'
...
def _build_cer_energy(cer_df: pd.DataFrame) -> pl.DataFrame:
    """Filter cer_resd_demand output to Petroleum Refining CIMS nodes."""
    pr = cer_df[cer_df['Node'].str.startswith('.Petroleum Refining')].copy()
    if pr.empty:
        return _empty_df()
...
def _build_crosswalk_emissions(crosswalk_df: pl.DataFrame) -> pl.DataFrame:
    """Filter nir_crosswalk_tables_cims output to Petroleum Refining CIMS branches."""
    pr = crosswalk_df.filter(pl.col('CIMS_Branch').str.contains(r'\.Petroleum Refining'))
    if pr.is_empty():
        return _empty_df()

    rows = []
    for row in pr.to_dicts():
...
def _build_nir_emissions(nir_df: pl.DataFrame) -> pl.DataFrame:
    """Extract per-gas NIR emissions for Petroleum Refining branches."""
    known_regions = set(_REGION_MAP.keys())
    pr = nir_df.filter(
        pl.col('CIMS Branch').str.contains(r'\.Petroleum Refining')
...
def main() -> pl.DataFrame:
    """Assemble petroleum refining calibration data and write one CSV per region."""
    print('=' * 60)
    print('PETROLEUM REFINING CALIBRATION')
    print('=' * 60)
    ...
    print('Filtering to regions with Petroleum Refining (see sector_region_map.csv)...')
    ...
    if allowed_regions:
        ...
        if dropped_count:
            print(f'  Dropped {dropped_count:,} rows for regions without Petroleum Refining')
    ...
    print(f'\n✅ Petroleum refining calibration complete')
