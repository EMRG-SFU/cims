"""
Extract ethanol calibration data and save to CIMS-formatted CSV files.
...
Emissions  (calibration_emissions_total from crosswalk; calibration_emissions_by_type from nir_to_cims)
    ...
    nir_to_cims.py                → per-gas kt per ethanol CIMS branch,
...
def main() -> pl.DataFrame:
    """Assemble ethanol calibration data and write one CSV per region."""
    print('=' * 60)
    print('ETHANOL CALIBRATION')
    print('=' * 60)
    ...
    print('Filtering to regions with Ethanol (see sector_region_map.csv)...')
    ...
    if allowed_regions:
        ...
        if dropped_count:
            print(f'  Dropped {dropped_count:,} rows for regions without Ethanol')
