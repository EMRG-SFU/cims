"""
Extract mining calibration data and save to CIMS-formatted CSV files.

Sources
-------
Emissions  (calibration_emissions_total from crosswalk; calibration_emissions_by_type from nir_to_cims)
    nir_crosswalk_tables_cims.py  → total tCO2e per mining CIMS branch
                                    5-year intervals (2000–2020);
                                    abbreviation regions (AB, BC, …)
    nir_to_cims.py                → per-gas kt per mining CIMS branch,
...
# ── configuration ─────────────────────────────────────────────────────────────
OUTPUT_DIR = BASE_PATH / 'calibration/mining'
...
def _build_cer_energy(cer_df: pd.DataFrame) -> pl.DataFrame:
    """Filter cer_resd_demand output to Mining CIMS nodes."""
    mng = cer_df[cer_df['Node'].str.startswith('.Mining')].copy()
    if mng.empty:
        return _empty_df()

    rows = []
    for _, row in mng.iterrows():
...
def _build_crosswalk_emissions(crosswalk_df: pl.DataFrame) -> pl.DataFrame:
    """Filter nir_crosswalk_tables_cims output to Mining CIMS branches."""
    mng = crosswalk_df.filter(pl.col('CIMS_Branch').str.contains(r'\.Mining'))
    if mng.is_empty():
        return _empty_df()

    rows = []
    for row in mng.to_dicts():
...
def _build_nir_emissions(nir_df: pl.DataFrame) -> pl.DataFrame:
    """Extract per-gas NIR emissions for Mining branches."""
    known_regions = set(_REGION_MAP.keys())
    mng = nir_df.filter(
        pl.col('CIMS Branch').str.contains(r'\.Mining')
...
def main() -> pl.DataFrame:
    """Assemble mining calibration data and write one CSV per region."""
    print('=' * 60)
    print('MINING CALIBRATION')
    print('=' * 60)
    ...
    print('Filtering to regions with Mining (see sector_region_map.csv)...')
    ...
    if allowed_regions:
        ...
        if dropped_count:
            print(f'  Dropped {dropped_count:,} rows for regions without Mining')
