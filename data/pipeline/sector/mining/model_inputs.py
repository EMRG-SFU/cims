...
def _build_feedstock_rows(heavy_ind: pl.DataFrame) -> pl.DataFrame:
    """
    Feedstock service rows for the Mining sector (CER vFsDmd-CIMS.csv, via
    cer_resd_demand.load_feedstock_demand()), tied to the same 'Mining'
    tonnes driver _build_total_rows() already uses.
    """
    feedstock_demand = _cer_resd_mod.load_feedstock_demand()
    feedstock_demand = feedstock_demand[feedstock_demand['Node'] == '.Mining']
    if feedstock_demand.empty:
        return pl.DataFrame()

    driver = heavy_ind.filter(pl.col('Variable') == 'Mining').with_columns(
