"""
Extract petroleum crude calibration data and save to CIMS-formatted CSV files.

Sources
-------
Emissions  (calibration_emissions_total from crosswalk; calibration_emissions_by_type from nir_to_cims)
    nir_crosswalk_tables_cims.py  → total tCO2e per petroleum crude CIMS branch
                                    5-year intervals (2000–2020);
                                    abbreviation regions (AB, BC, …)
    nir_to_cims.py                → per-gas kt per petroleum crude CIMS branch,
...
# ── configuration ─────────────────────────────────────────────────────────────
OUTPUT_DIR = BASE_PATH / 'calibration/petroleum_crude'
...
def _build_cer_energy(cer_df: pd.DataFrame) -> pl.DataFrame:
    """Filter cer_resd_demand output to Petroleum Crude CIMS nodes."""
    pc = cer_df[cer_df['Node'].str.startswith('.Petroleum Crude')].copy()
    if pc.empty:
        return _empty_df()
...
def _build_crosswalk_emissions(crosswalk_df: pl.DataFrame) -> pl.DataFrame:
    """Filter nir_crosswalk_tables_cims output to Petroleum Crude CIMS branches."""
    pc = crosswalk_df.filter(pl.col('CIMS_Branch').str.contains(r'\.Petroleum Crude'))
    if pc.is_empty():
        return _empty_df()

    rows = []
    for row in pc.to_dicts():
...
def _build_nir_emissions(nir_df: pl.DataFrame) -> pl.DataFrame:
    """Extract per-gas NIR emissions for Petroleum Crude branches."""
    known_regions = set(_REGION_MAP.keys())
    pc = nir_df.filter(
        pl.col('CIMS Branch').str.contains(r'\.Petroleum Crude')
...
def main() -> pl.DataFrame:
    """Assemble petroleum crude calibration data and write one CSV per region."""
    print('=' * 60)
    print('PETROLEUM CRUDE CALIBRATION')
    print('=' * 60)
    ...
    print('Filtering to regions with Petroleum Crude (see sector_region_map.csv)...')
    ...
    if allowed_regions:
        ...
        if dropped_count:
            print(f'  Dropped {dropped_count:,} rows for regions without Petroleum Crude')
    ...
    print(f'\n✅ Petroleum crude calibration complete')
