...
Fixed structural parameters
    ...
    Forestry sector to its Transport sub-service.

Activity demand  (service_provide levels)
    pipeline/source/activity/emissions_drivers.py  (called directly via main())
    Variables used:
      'Forestry'   → total tCO2e (region-level service_request)

    Forestry has no fractional sub-service splits in
    emissions_drivers. The sector→Transport passthrough (value=1) is a
    fixed structural parameter already present in the fixed data CSVs.
...
def _read_flattened_fixed_data() -> pl.DataFrame:
    """
    Flatten all Forestry fixed CSVs and return as a combined DataFrame.
    ...
    The fixed data encodes the full Forestry→Transport→{Diesel,Electric}
    structure, including the constant service_request of 1 from the
    Forestry sector node to its Transport sub-service.
    """
...
def _build_emission_rows(emissions: pl.DataFrame, valid_regions: list[str]) -> pl.DataFrame:
    ...
    Forestry has a single top-level tCO2e demand. The sector→Transport
...
def main() -> pl.DataFrame:
    """Assemble forestry model inputs and write one CSV per region."""
    print('=' * 60)
    print('FORESTRY MODEL INPUTS')
    print('=' * 60)
    ...
    _transport_branch = pl.col('Branch').str.ends_with('.Forestry.Transport')
    _header_params    = pl.col('Parameter').is_in(['service_provide', 'competition'])

    fixed_for_header       = fixed_str.filter(_for_branch & _header_params)
    fixed_for_tail         = fixed_str.filter(_for_branch & ~_header_params)
    ...
    fixed_rest             = fixed_str.filter(~_for_branch & ~_transport_branch)
