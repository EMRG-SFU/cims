"""
Extract hydrogen calibration data and save to CIMS-formatted CSV files.

Sources
-------
Emissions  (calibration_emissions_total from crosswalk; calibration_emissions_by_type from nir_to_cims)
    nir_crosswalk_tables_cims.py  → total tCO2e per hydrogen CIMS branch
                                    5-year intervals (2000–2020);
                                    abbreviation regions (AB, BC, …)
    nir_to_cims.py                → per-gas kt per hydrogen CIMS branch,
...
# ── configuration ─────────────────────────────────────────────────────────────
OUTPUT_DIR = BASE_PATH / 'calibration/hydrogen'
...
def _build_cer_energy(cer_df: pd.DataFrame) -> pl.DataFrame:
    """Filter cer_resd_demand output to Hydrogen CIMS nodes."""
    hyd = cer_df[cer_df['Node'].str.startswith('.Hydrogen')].copy()
    if hyd.empty:
        return _empty_df()
...
def _build_crosswalk_emissions(crosswalk_df: pl.DataFrame) -> pl.DataFrame:
    """Filter nir_crosswalk_tables_cims output to Hydrogen CIMS branches."""
    hyd = crosswalk_df.filter(pl.col('CIMS_Branch').str.contains(r'\.Hydrogen'))
    if hyd.is_empty():
        return _empty_df()

    rows = []
    for row in hyd.to_dicts():
...
def _build_nir_emissions(nir_df: pl.DataFrame) -> pl.DataFrame:
    """Extract per-gas NIR emissions for Hydrogen branches."""
    known_regions = set(_REGION_MAP.keys())
    hyd = nir_df.filter(
        pl.col('CIMS Branch').str.contains(r'\.Hydrogen')
...
def main() -> pl.DataFrame:
    """Assemble hydrogen calibration data and write one CSV per region."""
    print('=' * 60)
    print('HYDROGEN CALIBRATION')
    print('=' * 60)
    ...
    print('Filtering to regions with Hydrogen (see sector_region_map.csv)...')
    ...
    if allowed_regions:
        ...
        if dropped_count:
            print(f'  Dropped {dropped_count:,} rows for regions without Hydrogen')
