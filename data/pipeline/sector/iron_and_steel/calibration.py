"""
Extract iron and steel calibration data and save to CIMS-formatted CSV files.

Sources
-------
Emissions  (calibration_emissions_total from crosswalk; calibration_emissions_by_type from nir_to_cims)
    nir_crosswalk_tables_cims.py  → total tCO2e per iron and steel CIMS branch
                                    5-year intervals (2000–2020);
                                    abbreviation regions (AB, BC, …)
    nir_to_cims.py                → per-gas kt per iron and steel CIMS branch,
...
# ── configuration ─────────────────────────────────────────────────────────────
OUTPUT_DIR = BASE_PATH / 'calibration/iron_and_steel'
...
def _build_cer_energy(cer_df: pd.DataFrame) -> pl.DataFrame:
    """Filter cer_resd_demand output to Iron and Steel CIMS nodes."""
    ias = cer_df[cer_df['Node'].str.startswith('.Iron and Steel')].copy()
    if ias.empty:
        return _empty_df()

    rows = []
    for _, row in ias.iterrows():
...
def _build_crosswalk_emissions(crosswalk_df: pl.DataFrame) -> pl.DataFrame:
    """Filter nir_crosswalk_tables_cims output to Iron and Steel CIMS branches."""
    ias = crosswalk_df.filter(pl.col('CIMS_Branch').str.contains(r'\.Iron and Steel'))
    if ias.is_empty():
        return _empty_df()

    rows = []
    for row in ias.to_dicts():
...
def _build_nir_emissions(nir_df: pl.DataFrame) -> pl.DataFrame:
    """Extract per-gas NIR emissions for Iron and Steel branches."""
    known_regions = set(_REGION_MAP.keys())
    ias = nir_df.filter(
        pl.col('CIMS Branch').str.contains(r'\.Iron and Steel')
...
def main() -> pl.DataFrame:
    """Assemble iron and steel calibration data and write one CSV per region."""
    print('=' * 60)
    print('IRON AND STEEL CALIBRATION')
    print('=' * 60)
    ...
    print('Filtering to regions with Iron and Steel (see sector_region_map.csv)...')
    ...
    if allowed_regions:
        ...
        if dropped_count:
            print(f'  Dropped {dropped_count:,} rows for regions without Iron and Steel')
    ...
    for region in regions:
        ...
        if not (region_df['Value'].cast(pl.Float64, strict=False).fill_null(0) != 0).any():
            continue
        out_path  = OUTPUT_DIR / f'iron_and_steel_{region.lower()}.csv'
        region_df.write_csv(out_path)
        print(f'  Wrote {len(region_df):,} rows → {out_path.name}')

    print(f'\n✅ Iron and steel calibration complete')
