"""
Extract chemical products calibration data and save to CIMS-formatted CSV files.

Sources
-------
Emissions  (calibration_emissions_total from crosswalk; calibration_emissions_by_type from nir_to_cims)
    nir_crosswalk_tables_cims.py  → total tCO2e per chemical products CIMS branch
                                    5-year intervals (2000–2020);
                                    abbreviation regions (AB, BC, …)
    nir_to_cims.py                → per-gas kt per chemical products CIMS branch,
...
# ── configuration ─────────────────────────────────────────────────────────────
OUTPUT_DIR = BASE_PATH / 'calibration/chemical_products'
...
def _build_cer_energy(cer_df: pd.DataFrame) -> pl.DataFrame:
    """Filter cer_resd_demand output to Chemical Products CIMS nodes."""
    chem = cer_df[cer_df['Node'].str.startswith('.Chemical Products')].copy()
    if chem.empty:
        return _empty_df()
...
def _build_crosswalk_emissions(crosswalk_df: pl.DataFrame) -> pl.DataFrame:
    """Filter nir_crosswalk_tables_cims output to Chemical Products CIMS branches."""
    chem = crosswalk_df.filter(pl.col('CIMS_Branch').str.contains(r'\.Chemical Products'))
    if chem.is_empty():
        return _empty_df()

    rows = []
    for row in chem.to_dicts():
...
def _build_nir_emissions(nir_df: pl.DataFrame) -> pl.DataFrame:
    """Extract per-gas NIR emissions for Chemical Products branches."""
    known_regions = set(_REGION_MAP.keys())
    chem = nir_df.filter(
        pl.col('CIMS Branch').str.contains(r'\.Chemical Products')
...
def main() -> pl.DataFrame:
    """Assemble chemical products calibration data and write one CSV per region."""
    print('=' * 60)
    print('CHEMICAL PRODUCTS CALIBRATION')
    print('=' * 60)
    ...
    print('Filtering to regions with Chemical Products (see sector_region_map.csv)...')
    ...
    if allowed_regions:
        ...
        if dropped_count:
            print(f'  Dropped {dropped_count:,} rows for regions without Chemical Products')
    ...
    print(f'\n✅ Chemical products calibration complete')
