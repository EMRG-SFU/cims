...
def _build_feedstock_rows(heavy_ind: pl.DataFrame) -> pl.DataFrame:
    """
    Feedstock service rows for the Chemical Products sector (CER
    vFsDmd-CIMS.csv, via cer_resd_demand.load_feedstock_demand()), tied to
    the same 'Chemical Product' tonnes driver _build_total_rows() already uses.
    """
    feedstock_demand = _cer_resd_mod.load_feedstock_demand()
    feedstock_demand = feedstock_demand[feedstock_demand['Node'] == '.Chemical Products']
    if feedstock_demand.empty:
        return pl.DataFrame()

    driver = heavy_ind.filter(pl.col('Variable') == 'Chemical Product').with_columns(
    ...
    output = (
        pl.concat(
            [total_rows, fixed_sector_service_provide, fixed_sector_competition,
             price_rows, subprod_rows, feedstock_rows, fixed_rest],
