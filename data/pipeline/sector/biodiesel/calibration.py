...
def _build_cer_energy(cer_df: pd.DataFrame) -> pl.DataFrame:
    """Filter cer_resd_demand output to Biodiesel CIMS nodes."""
    bio = cer_df[cer_df['Node'].str.startswith('.Biodiesel')].copy()
    if bio.empty:
        return _empty_df()
...
def _build_crosswalk_emissions(crosswalk_df: pl.DataFrame) -> pl.DataFrame:
    """Filter nir_crosswalk_tables_cims output to Biodiesel CIMS branches."""
    bio = crosswalk_df.filter(pl.col('CIMS_Branch').str.contains(r'\.Biodiesel'))
    if bio.is_empty():
        return _empty_df()

    rows = []
    for row in bio.to_dicts():
...
def _build_nir_emissions(nir_df: pl.DataFrame) -> pl.DataFrame:
    """Extract per-gas NIR emissions for Biodiesel branches."""
    known_regions = set(_REGION_MAP.keys())
    bio = nir_df.filter(
        pl.col('CIMS Branch').str.contains(r'\.Biodiesel')
