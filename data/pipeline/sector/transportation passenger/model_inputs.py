...
# ── configuration ──────────────────────────────────────────────────────────────
FIXED_INPUT_DIR = BASE_PATH / 'raw_data/fixed_data/Transportation Passenger'
OUTPUT_DIR      = BASE_PATH / 'model_inputs/model/transportation passenger'
...
def _read_flattened_fixed(region: str) -> pl.DataFrame:
    ...
    fixed_path = FIXED_INPUT_DIR / f'transportation passenger_{region}.csv'
    with tempfile.TemporaryDirectory() as tmp:
        out_file = Path(tmp) / f'transportation passenger_{region}.csv'
...
def main() -> dict[str, pl.DataFrame]:
    ...
    for region, template in sorted(FIXED_TEMPLATE.items()):
        ...
        try:
            ...
            out_path = OUTPUT_DIR / f'transportation passenger_{region}.csv'
