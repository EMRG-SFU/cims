"""
Extract metal smelting calibration data and save to CIMS-formatted CSV files.

Sources
-------
Emissions  (calibration_emissions_total from crosswalk; calibration_emissions_by_type from nir_to_cims)
    nir_crosswalk_tables_cims.py  → total tCO2e per metal smelting CIMS branch
                                    5-year intervals (2000–2020);
                                    abbreviation regions (AB, BC, …)
    nir_to_cims.py                → per-gas kt per metal smelting CIMS branch,
...
def _build_nir_emissions(nir_df: pl.DataFrame) -> pl.DataFrame:
    """Extract per-gas NIR emissions for Metal Smelting branches."""
    known_regions = set(_REGION_MAP.keys())
    ms = nir_df.filter(
        pl.col('CIMS Branch').str.contains(r'\.Metal Smelting')
...
def main() -> pl.DataFrame:
    """Assemble metal smelting calibration data and write one CSV per region."""
    print('=' * 60)
    print('METAL SMELTING CALIBRATION')
    print('=' * 60)
    ...
    print('Filtering to regions with Metal Smelting (see sector_region_map.csv)...')
    ...
    if allowed_regions:
        ...
        if dropped_count:
            print(f'  Dropped {dropped_count:,} rows for regions without Metal Smelting')
    ...
    for region in regions:
        ...
        if not (region_df['Value'].cast(pl.Float64, strict=False).fill_null(0) != 0).any():
            continue
        out_path  = OUTPUT_DIR / f'metal_smelting_{region.lower()}.csv'
        region_df.write_csv(out_path)
        print(f'  Wrote {len(region_df):,} rows → {out_path.name}')

    print(f'\n✅ Metal smelting calibration complete')
