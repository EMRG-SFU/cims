...
Fixed structural parameters
    raw_data/fixed_data/metal_smelting/metal_smelting_{region}.csv
...
4. service_request  — Primary Metals → each subproduct (from heavy_industry)
...
OUTPUT_DIR      = BASE_PATH / 'model_inputs/model/metal_smelting'
...
def _build_total_rows(heavy_ind: pl.DataFrame) -> pl.DataFrame:
    ...
    return df.select([
        ...
        (pl.lit('CIMS.CAN.') + pl.col('Region') + pl.lit('.Metal Smelting')).alias('Target'),
        pl.col('Source'),
