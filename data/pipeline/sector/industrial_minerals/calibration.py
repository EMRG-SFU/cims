"""
Extract industrial minerals calibration data and save to CIMS-formatted CSV files.

Sources
-------
Emissions  (calibration_emissions_total from crosswalk; calibration_emissions_by_type from nir_to_cims)
    nir_crosswalk_tables_cims.py  → total tCO2e per industrial minerals CIMS branch
                                    5-year intervals (2000–2020);
                                    abbreviation regions (AB, BC, …)
    nir_to_cims.py                → per-gas kt per industrial minerals CIMS branch,
...
# ── configuration ─────────────────────────────────────────────────────────────
OUTPUT_DIR = BASE_PATH / 'calibration/industrial_minerals'
...
SECTOR_NAME = 'Industrial Minerals'
...
def _build_cer_energy(cer_df: pd.DataFrame) -> pl.DataFrame:
    """Filter cer_resd_demand output to Industrial Minerals CIMS nodes."""
    im = cer_df[cer_df['Node'].str.startswith('.Industrial Minerals')].copy()
    if im.empty:
        return _empty_df()
...
def _build_crosswalk_emissions(crosswalk_df: pl.DataFrame) -> pl.DataFrame:
    """Filter nir_crosswalk_tables_cims output to Industrial Minerals CIMS branches."""
    im = crosswalk_df.filter(pl.col('CIMS_Branch').str.contains(r'\.Industrial Minerals'))
    if im.is_empty():
        return _empty_df()

    rows = []
    for row in im.to_dicts():
...
def _build_nir_emissions(nir_df: pl.DataFrame) -> pl.DataFrame:
    """Extract per-gas NIR emissions for Industrial Minerals branches."""
    known_regions = set(_REGION_MAP.keys())
    im = nir_df.filter(
        pl.col('CIMS Branch').str.contains(r'\.Industrial Minerals')
...
def main() -> pl.DataFrame:
    """Assemble industrial minerals calibration data and write one CSV per region."""
    print('=' * 60)
    print('INDUSTRIAL MINERALS CALIBRATION')
    print('=' * 60)
    ...
    print('Filtering to regions with Industrial Minerals (see sector_region_map.csv)...')
    ...
    if allowed_regions:
        ...
        if dropped_count:
            print(f'  Dropped {dropped_count:,} rows for regions without Industrial Minerals')
    ...
    print(f'\n✅ Industrial minerals calibration complete')
