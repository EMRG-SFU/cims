...
Same estimation logic as the original nir_walkthrough_tables.py, but the
...
Usage:
    python nir_walkthrough_tables_cims.py

Outputs:
    nir_walkthrough_cims.csv
"""
...
OUTPUT   = 'C:/cims/data/processed_data/eccc/nir/nir_walkthrough_cims_leaves.csv'
...
#
# CSV columns: NIR_Sub_Sector, NIR_Sub_Sub_Sector, NIR_Category,
#              NIR_Sub_Category, NIR_Sub_Sub_Category, CIMS_Path
#
...
def load_nir_to_cims_map(path):
    ...
    (Sub_Sector, Sub_Sub_Sector, Category, Sub_Category, Sub_Sub_Category)
    with a list of CIMS_Path strings as values.
    """
    ...
    for _, row in df_map.iterrows():
        key = (
            row['NIR_Sector'],
            row['NIR_Sub_Sector'],
            row['NIR_Sub_Sub_Sector'],
            row['NIR_Category'],
            row['NIR_Sub_Category'],
            row['NIR_Sub_Sub_Category'],
        )
        mapping.setdefault(key, []).append(row['CIMS_Path'])
    return mapping
...
# ── Row offset → (Sector, Sub_Sector[, Sub_Sub_Sector]) ───────────────────────
SUBSECTOR_MAP = {
    3:  ('Oil and Gas', 'Natural Gas Production and Processing'),
    5:  ('Oil and Gas', 'Conventional Light Oil Production'),
    6:  ('Oil and Gas', 'Conventional Heavy Oil Production'),
    7:  ('Oil and Gas', 'Frontier Oil Production'),
    9:  ('Oil and Gas', 'Oil Sands Mining and Extraction'),
    10: ('Oil and Gas', 'Oil Sands In-Situ'),
    11: ('Oil and Gas', 'Oil Sands Upgrading'),
    12: ('Oil and Gas', 'Oil, Natural Gas and CO2 Transmission'),
    14: ('Oil and Gas', 'Petroleum Refining'),
    15: ('Oil and Gas', 'Natural Gas Distribution'),
    16: ('Electricity', 'Electricity'),
    19: ('Transport', 'Passenger', 'Cars, Light Trucks and Motorcycles'),
    20: ('Transport', 'Passenger', 'Bus, Rail and Aviation'),
    22: ('Transport', 'Freight',   'Heavy Duty Trucks, Rail'),
    23: ('Transport', 'Freight',   'Aviation and Marine'),
    ...
    32: ('Heavy Industry', 'Chemicals and Fertilizers'),
    ...
    41: ('Waste', 'Solid Waste'),
    42: ('Waste', 'Wastewater'),
    43: ('Waste', 'Waste Incineration'),
    44: ('Coal Production', 'Coal Production'),
...
for region in ['CAN'] + ALL_REGIONS:
    fname = f'{DATA_DIR}{region.lower()}_walkthrough_nir.csv'
...
for region in ALL_REGIONS:
    for year in YEARS:
        for offset, subsector_info in SUBSECTOR_MAP.items():
            # Unpack sub-sector info (2 or 3 levels)
            sector, sub_sector, *rest = subsector_info
            sub_sub_sector = rest[0] if rest else ''
            ...
            for col, col_info in COL_INFO.items():
                ...
                # (sector, sub_sector, sub_sub_sector, category, sub_cat, sub_sub_cat)
                ...
                for cims_path in cims_paths:
                    branch = f"CIMS.CAN.{region}.{cims_path}"
                    records.append({
                        'CIMS_Branch': branch,
                        'Region':      region,
                        'Year':        year,
