...
def get_node_cols(mdf, first_data_col_name="Node", extra_col="Demand?"):
    ...
    extra_col_idx = mdf.columns.get_loc(extra_col)
    relevant_columns = mdf.columns[node_col_idx:].append(pd.Index([mdf.columns[extra_col_idx]]))
    ...
    node_cols = mdf.columns[node_col_idx:][:first_year_idx].append(pd.Index([mdf.columns[extra_col_idx]]))
