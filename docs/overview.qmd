...
CIMS[^overview-1] is an integrated set of economic, energy, and materials models designed to provide information to policy makers on the likely response of firms and households to policies that influence their technology acquisition and technology use decisions.[^overview-2] Thus, it is sometimes described as a technology simulation model, which seeks to reflect how people actually behave rather than how they ought to behave.[^overview-3] CIMS is able to provide policy makers with information on: (1) the ability of their policies to achieve specific objectives (like greenhouse gas (GHG) emissions reductions), (2) the likely costs of achieving these objectives, and (3) the uncertainties associated with the model's simulations. CIMS is maintained and operated by the Energy and Materials Research Group at the School of Resource and Environmental Management at Simon Fraser University.

[^overview-1]: CIMS originally stood for Canadian Integrated Modelling System but, as the model is now being applied to other countries, the acronym is now used as a proper name.

[^overview-2]: Testing

[^overview-3]: More testing
...
<mark>TODO: Model licenses (MIT & CC4.0)</mark>

<mark>TODO: Split repository into code and data</mark>

## Model Scale and Scope

CIMS covers the entire Canadian economy and can connect to an aggregated representation of the US economy. The Canadian version of CIMS currently includes six provinces and an aggregation of the Atlantic Provinces.\[\^4\] [Table 1](#_Ref61957786) illustrates the sectors and regions covered in CIMS.

<mark>TODO: Separate out Atlantic provinces (medium priority) and Territories (lower priority)</mark>

| **Sectors**             | **BC** | **AB** | **SK** | **MB** | **ON** | **QC** | **AT** |
|:--------|:-------:|:-------:|:-------:|:-------:|:-------:|:-------:|:-------:|
| Residential             |   X    |   X    |   X    |   X    |   X    |   X    |   X    |
| Commercial              |   X    |   X    |   X    |   X    |   X    |   X    |   X    |
| Transportation Personal |   X    |   X    |   X    |   X    |   X    |   X    |   X    |
| Transportation Freight  |   X    |   X    |   X    |   X    |   X    |   X    |   X    |
| Mining                  |   X    |        |   X    |   X    |   X    |   X    |   X    |
| Coal Mining             |   X    |   X    |   X    |        |        |        |   X    |
| Natural Gas Extraction  |   X    |   X    |   X    |   X    |   X    |   X    |   X    |
...
<mark>TODO: Include major cities as separate nodes under provinces?</mark>
...
<mark>TODO: Review Winner-take-all function and decide if necessary</mark>
...
<mark>TODO: Model currently equilibrates only through prices. Could we also find quantity equilibrium for cap system?</mark>
