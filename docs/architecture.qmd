...
CIMS covers the entire Canadian economy and can connect to aggregated representations of other regions (e.g., United States, Europe, or Rest of World). @tbl-sectors-regions-dynamic illustrates the sectors and regions currently covered in CIMS.[^arch-territories]

[^arch-territories]: Canadian territories are not currently represented in CIMS.

```{python}
#| label: tbl-sectors-regions-dynamic
#| tbl-cap: "Sectors represented by region"
#| echo: false
...
# Pandoc allocates pipe-table column widths in proportion to the dash
# counts in the separator row. Giving the Sectors column a much higher
# weight (sector_weight) than each region column (region_weight) lets us
# fix the first column at a wide relative size while every region column
# shares the remaining width equally, regardless of how many regions are
# active in this render.
sector_weight = 40
region_weight = 4
lines = [
    "| **Sectors** | " + " | ".join(_format_header(c) for c in column_order) + " |",
    "|:" + "-" * sector_weight + "|"
    + "|".join([":" + "-" * region_weight + ":" for _ in column_order])
    + "|",
]
for sector in sector_order:
    # Each "on" cell carries a literal "X" wrapped in a .cell-on span.
    # In HTML, style.css uses `td:has(.cell-on) { ... }` to shade the
    # cell and `.cell-on { color: transparent; }` to hide the "X", so
    # only the shading shows. In PDF (no CSS), the "X" renders normally
    # and acts as the cell marker.
...
![General model structure](images/general_model_structure.svg){#fig-general_model_structure .lightbox width=70%}

## Simulation: Six Steps
...
6.  Output is provided for new and total capital stock, energy use, and trade flows, GHG emissions, levelized lifecycle costs by service and technology,