...
import CIMS.data_processing.source.activity.light_industrial as _light_industrial_mod
...
def _build_feedstock_rows(light_ind: pl.DataFrame) -> pl.DataFrame:
    """
    Feedstock service rows for the Light Industrial sector (CER
    vFsDmd-CIMS.csv, via cer_resd_demand.load_feedstock_demand()), tied to
    the same 'Light Industrial' $M GDP driver _build_total_rows() already uses.
    """
    feedstock_demand = _cer_resd_mod.load_feedstock_demand()
    feedstock_demand = feedstock_demand[feedstock_demand['Node'] == '.Light Industrial']
    if feedstock_demand.empty:
        return pl.DataFrame()

    driver = light_ind.filter(pl.col('Variable') == 'Light Industrial').with_columns(
...
def main() -> pl.DataFrame:
    ...
    feedstock_rows = _build_feedstock_rows(light_ind)
    ...
    output = (
        pl.concat(
            [total_rows, fixed_sector_competition,
             price_rows, fixed_mfg_header, mfg_split_rows, feedstock_rows, fixed_rest],
