...
import CIMS.data_processing.source.activity.coal_mining as _coal_mining_mod
...
def _build_feedstock_rows(coal: pl.DataFrame) -> pl.DataFrame:
    """
    Feedstock service rows for the Coal Mining sector (CER vFsDmd-CIMS.csv,
    via cer_resd_demand.load_feedstock_demand()), tied to the same
    'Coal Mining' kt driver _build_total_rows() already uses.
    """
    feedstock_demand = _cer_resd_mod.load_feedstock_demand()
    feedstock_demand = feedstock_demand[feedstock_demand['Node'] == '.Coal Mining']
    if feedstock_demand.empty:
        return pl.DataFrame()

    driver = coal.filter(pl.col('Variable') == 'Coal Mining').with_columns(
...
def main() -> pl.DataFrame:
    ...
    feedstock_rows = _build_feedstock_rows(coal)
    ...
    output = (
        pl.concat(
            [total_rows, fixed_sector_competition, price_rows, feedstock_rows,
