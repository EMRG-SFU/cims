...
# Need to call this here before importing pyplot. If we don't do this, on the mac it tries
# to create windows when doing the plotting, and this isn't allowed unless its on the main
# thread (which this isn't, being run from the flask server). Then it explodes and crashes.
matplotlib.use('Agg')
...
# This is to try save the matplotlib figure to a bytestream. The plotly version of the
# plots has its own module for doing this (plotly.io).
import io

from . import utility_functions as uf
from .ensure_legend_fits import ensure_legend_fits
...
def plot_ms_for_node_stack(model, targetNode):
    def maybeFloat(x):
        if x is None:
            return(None)
    ...
    fig = make_subplots(rows=1, cols=2, subplot_titles=("CIMS calc MS","Counterfactual MS"))
    ...
    # Suggestion from Lumo about how to control the size and location
    # of the legend to keep it all readable.
    # ::UPDATE:: no this doesn't change anything. Legend is still
    #            cut off by the overall height of the figure.
    # Make the font size tiny doesn't change how many things fit
    # as that seems to be mostly constrained by the sample colour
    # swatches.
    #fig.update_layout(
    #    legend=dict(
    #        orientation='v',
    ...
    fig = ensure_legend_fits(fig, verbose=True)

    #img_bytes = pio.to_image(fig, format='png', width=800, height=400)
    img_bytes = pio.to_image(fig, format='png')
    img_bytes_ready = base64.b64encode(img_bytes).decode("utf-8")
    data_uri = f"data:image/png;base64,{img_bytes_ready}"
    return(data_uri)
