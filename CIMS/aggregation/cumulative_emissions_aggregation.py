...
def _aggregate_cumulative_emission_rates(model, year, emissions_for_aggregation, base_emissions_class):
    ...
    for emission_dict in emissions_for_aggregation:
        ...
        if parent_tech is not None:
            total_market_share = model.get_param(PARAM.total_market_share, parent_node, year,
                                                 tech=parent_tech)
...
def _find_cumulative_rate_via_request_provide_edge(model, agg_info, year, cumulative_rate_param, base_emissions_class):
    ...
    req_ratio = model.get_param(PARAM.service_requested, parent_node, year, tech=parent_tech, context=child_node)
