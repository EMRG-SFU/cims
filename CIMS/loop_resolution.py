...
def min_distance_from_root(list_of_cycles, distances_from_root, **kwargs):
    ...
    candidates = {node: distances_from_root[node] for cycle in sorted(list_of_cycles) for node in cycle}
...
def aggregation_resolution(list_of_cycles, distances_from_root, **kwargs):
    # TODO: Document function
    lists_of_edges = [list(zip(nodes, (nodes[1:]+nodes[:1]))) for nodes in sorted(list_of_cycles)]
