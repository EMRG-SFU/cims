...
def make_or_update_nodes(graph, node_dfs, tech_dfs):
    ...
    for node in tech_dfs:
        ...
        for tech in tech_dfs[node]:
            new_graph = graph_utils.add_tech_data(graph, node, tech_dfs, tech)

    # Return the graph
    return new_graph


def _copy_graph_and_df(graph, node_dfs, current_node):
...
def _init_node(graph, current_node_df, current_node):
    ...
    if TREE_INDEX not in graph.nodes[current_node]:
...
def _set_node_constants(graph, current_node_df, current_node):
    ...
    graph = add_node_constant(graph, current_node_df, current_node, COMP_TYPE, required=True)
    current_node_df = current_node_df[current_node_df['Parameter'] != COMP_TYPE]
...
def _set_node_region_sector(graph, current_node_df, current_node):
    ...
    if graph.nodes[current_node][COMP_TYPE] not in ['root', 'region']:
        sector_list = []
...
def _build_cost_curve(graph, current_node_df, current_node):
    comp_type = graph.nodes[current_node][COMP_TYPE]
